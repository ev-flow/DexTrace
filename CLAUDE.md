...
Tests mirror the source tree: `src/dextrace/<pkg>/<module>.py` → `tests/<pkg>/test_<module>.py`. Multi-module / end-to-end tests live under `tests/integrations/`. Shared fixtures (synthetic DEX builders, `.dex` samples) live under `tests/fixtures/`.

## Subsystem → test mapping

| Area changed | Tests to run |
|---|---|
| CLI | `pytest tests/cli/ tests/integrations/test_smoke.py` |
| APK reader / metadata | `pytest tests/core/test_apk_reader.py tests/core/test_apk_metadata.py` |
| Manifest | `pytest tests/core/test_manifest_parser.py` |
| DEX structure | `pytest tests/core/test_dex_parser.py tests/core/test_dex_header.py` |
| API extraction | `pytest tests/core/test_dex_api_extractor.py` |
| Dalvik / disassembly | `pytest tests/dalvik/` |
| VM core (engine, heap, hierarchy, signals, trace) | `pytest tests/vm/` |
| VM opcode handlers | `pytest tests/vm/handlers/` |
| Android API stubs | `pytest tests/vm/android_stubs/` |
| VM end-to-end (real `.dex` fixtures) | `pytest tests/integrations/test_vm_run_*.py tests/integrations/test_vm_trace_*.py` |
| CLI error handling regressions | `pytest tests/integrations/test_vm_error_handling_regression.py` |
